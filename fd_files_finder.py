...
def count_total_files(search_path: str) -> int:
    ...
    try:
        ...
        cmd = ["fd", "--type", "f", "--hidden", "--no-ignore"] + exclusion_args + ["-j", str(num_cores), ".", search_path]
...
def run_unified_scan(file_patterns: List[str], extensions: List[str], search_path: str) -> List[str]:
    ...
    try:
        ...
        num_cores = os.cpu_count()
        exclusion_args = get_exclusion_args()
        base_cmd = ["fd", "--type", "f", "--hidden", "--no-ignore"] + exclusion_args + ["-j", str(num_cores), "--absolute-path"]
        full_cmd = base_cmd + ["--glob", combined_pattern, ".", search_path]

        total_patterns = len(extensions) + len(file_patterns)
        print(f"🔍 Scanning for files ({len(extensions)} extensions, {len(file_patterns)} patterns, {total_patterns} total)...")
...
def find_matching_files(file_patterns: List[str], extensions: List[str], search_path: str = str(Path.home())) -> List[str]:
...
Examples:
  ...
  python3 find_secrets_unified.py --path /usr/local  # Search with --path flag
    ...
    parser.add_argument(
        '--path', '-p',
        dest='search_path_alt',
        help='Alternative way to specify search path'
    )
